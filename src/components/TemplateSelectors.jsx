  ...
  const templates = [
    {
      id: "classic",
      name: "Classic",
      preview: "A clean and traditional resume template with a focus on simplicity and readability."
    },
    {
      id: "modern",
      name: "Modern",
      preview: "A sleek and contemporary resume template with bold typography and a unique layout."
    },
    {
      id: "minimal",
      name: "Minimal",
      preview: "A minimalist resume template with a clean design and ample white space for a professional look."
    }
  ]
        ...
        <div className="absolute top-full w-80 p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm">
          {templates.map((template) => (
            <div key={template.id} onClick={() => {onChange(template.id) 
                setIsOpen(false)
              }} className={`relative p-3 border rounded-md cursor-pointer transition
                 ${ selectedTemplate === template.id ? 'border-blue-500 bg-blue-100': 'border-gray-300 hover:border-gray-400 hover:bg-gray-100' }`}>
